...
> **知道你下一步为什么而学。**
>
> 从你的目标、最近的学习和现实时间出发，找到现在值得学的一小步。
...
学程面向有学习目标、正在做项目，却经常卡在“下一步学什么”的自学者。通过对话积累你的目标、限制与学习证据，提出一个具体行动，解释为什么现在值得做，并在你的反馈中调整。

**先聊清楚，再走一步；建议有依据，决定权在你。** 当前为早期可试用版本，自由对话需要在设置中接入自己的模型 API Key；未接入时仅提供有限的本地规则交互。

## 它解决什么

资源很多，时间有限。难的是判断：这件事和我的目标有什么关系？我现在做得了吗？做完之后，下一步该如何调整？

学程希望成为一个逐渐了解你的学习引路人：每次优先推进一件事，说明理由，允许讨论，并用执行反馈修正后续建议。
...
你说：“我在做一个 Web 项目，但网络基础很弱，今天只有 20 分钟。”

学程建议先理解 TCP 和 UDP：这会帮助你理解项目中的通信选择。先听一个例子，再用自己的话解释适用场景。如果今天很累，可以缩短为一个例子，或讨论改期。解释中暴露的疑问，应成为下一次建议的依据。

以下是目标体验示意，并非真实用户效果或模型能力保证。
...
| 自带 API Key 接入云端模型（BYOK） | 已有配置入口，依厂商兼容性而定 | 测试连接、获取与选择模型；需用户授权 |
| 后台主动通知、跨端同步 | 尚未完整交付 | 当前以设备本地保存为主 |
