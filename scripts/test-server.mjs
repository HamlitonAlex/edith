...
try{
 ...
 const [home,styles,icons,iconFont,avatar,manifest,legacy,module,agent,traversal]=await Promise.all([
  fetch(`${origin}/`),fetch(`${origin}/iphone.css`),fetch(`${origin}/phosphor-icons.css`),fetch(`${origin}/assets/phosphor/regular.woff2`),fetch(`${origin}/assets/companion-default.png`),
 ...
 assert.equal(icons.status,200);assert.match(icons.headers.get("content-type"),/text\/css/);await icons.text();
 assert.equal(iconFont.status,200);assert.equal(iconFont.headers.get("content-type"),"font/woff2");await iconFont.arrayBuffer();
